...
from functions_pet import *
import tkinter as root
import menus
import functions_pet


class Aplication_pet(functions):
    def __init__(self):
        self.root = Tk()
        self.tela()
        self.frames_tela()
    ...
    def tela(self):
        self.root.title("Cadastro de pet")
    ...
    def widgets_frame1(self):
        ...
        self.bt_buscar = Button(self.frame1, text="Buscar",
                                bg="#583bbf", fg="white", font=('verdana', 8, 'bold'), command=self.Buscar_pets)
        ...
        self.bt_novo = Button(self.frame1, text="Novo",
                              bg="#583bbf", fg="white", font=('verdana', 8, 'bold'), command=self.add_pets)
        self.bt_novo.place(relx=0.6, rely=0.1, relwidth=0.1, relheight=0.15)

        # Botão Altera
        self.bt_alterar = Button(self.frame1, text="Alterar",
                                 bg="#583bbf", fg="white", font=('verdana', 8, 'bold'), command=self.alterar_pets)
        self.bt_alterar.place(relx=0.7, rely=0.1, relwidth=0.1, relheight=0.15)

        # Botão Apagar
        self.bt_apagar = Button(self.frame1, text="Apagar",
                                bg="#583bbf", fg="white", font=('verdana', 8, 'bold'), command=self.deleta_pets)
        ...
        self.lb_raca = Label(self.frame1, text="Raça",
                                  bg="white", fg="#583bbf", font=('verdana', 10, 'bold'))
        self.lb_raca.place(relx=0.35, rely=0.35)

        self.entry_raca = Entry(self.frame1,
                                     bg="white", fg="#583bbf", font=('verdana', 10, 'bold'))
        self.entry_raca.place(relx=0.35, rely=0.45, relwidth=0.3)

        # label e entry - Cidade -----------------------
        self.lb_dtnasc = Label(self.frame1, text="Data de Nascimento",
                                 bg="white", fg="#583bbf", font=('verdana', 10, 'bold'))
        self.lb_dtnasc.place(relx=0.05, rely=0.6)

        self.entry_dtnasc = Entry(self.frame1,
                                    bg="white", fg="#583bbf", font=('verdana', 10, 'bold'))
        self.entry_dtnasc.place(relx=0.05, rely=0.7, relwidth=0.2)
    ...
    def grid_pets(self):
        ...
        self.lista_grid.heading("#3", text='RAÇA')
        self.lista_grid.heading("#4", text='DATA DE NASCIMENTO')
